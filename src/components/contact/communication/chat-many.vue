<template>
  <div>
    <div class="tab_content">
      ...
      <section class="chat-box" :style="{backgroundColor: backgroundColor}" @click="closeDisplay">
        <section class="chat-list chat-many-list">
          <ul id="ullMany">
          </ul>
        </section>
      </section>
    ...
    <div class="chat-footer">
      <div class="footer">
        ...
        <div class="somechoice" style="display: inline-block;margin-left: 1.4rem;">
          ...
          <span @click="showFont" style="left: 1.2rem;">字体<i class="icon-show"></i></span>
...
<script>
  ...
  export default{
    ...
    data() {
      return {
        chatUsers:this.$route.params.chatUsers,
        chatIps:this.$route.params.chatIps,
        chatIds:this.$route.params.chatIds,
    ...
    mounted(){
      console.log("多人聊天页面传参");
      console.log(this.chatUsers);
      console.log(this.chatIps);
    ...
    methods: {
      chatManyP(){
        let msg = {
          ...
          fontStyle:"",
          isReply:true,
          chatMany: this.chatUsers, /*成员昵称-职位*/
          chatIps:this.chatIps,
        ...
        for (let i = 0; i < this.chatIds.length; i++) {
          msg.identity+=this.chatIds[i];
        }
        $('.Main').myEmoji(this.myself, "", this.chatIps, msg);
      },
      checkoutRecordM(){
        let _this = this;
        let identity = "";
        ...
        for (let i = 0; i < this.chatIds.length; i++) {
          identity+=this.chatIds[i];
        }
        new Promise(function(resolve){
          db.queryConfig({identity: identity},function(err,res){
            if(err){
              console.log('failed with:' + err.message)
            }else{
              console.log("查询多人聊天历史记录");
              console.log(res);
              resolve(res);
            }
          })
        }).then(function(value){  //时间数组
          io.historyChat(value,$("#ullMany"),_this.myself,false,_this.chatUsers);
        }).then(function(){
          io.scrollBottom("ullMany");
          db.update({identity: identity},{ $set: { isRead: 'Y' } }, { multi: true },function(err,res){
            if(err){
      ...
      audioPlay(){
        $(".icon-receivevoice").click(function(){
          ...
          sound.on('end', function () {
            $(".icon-receivevoice").removeClass("active");
          });
          sound.play();
        });
      },
      ...
      sendPictrue() {
        ...
        $(".icon-takepic").on("change","input[type='file']",function(){
          ...
          if(filePath.indexOf("jpg")!=-1 || filePath.indexOf("png")!=-1){
            ...
            io.sendFile(_this.$store.state.otherIP,photoPath,"picture",_this.myself.uniqueId,_this.user.uniqueId,_this.myself.image,
            ...
            io.sendPhotoTemplate(ul,"","",num,filePath[0],msg.isFail,"", "",false,_this.chatUsers);
            }else{
            _this.$message.error("拍照上传有误，请重新拍照");
            return false
          }
      ...
      filePiker(){
        ...
        GeoBOS.filePicker.pickFile( function (err, filePath) {
          ...
          let msg = {
            ...
            chatMany: _this.chatUsers, /*成员昵称-职位*/
            /* chatIps:_this.chatIps, /!*成员ip*!/
             chatIds:_this.chatIds, */
            uuid : generateUUID.creatUuid(),
            msgStatus : "init",
            moduleUser:_this.myself.uniqueId,
            num:'',//模板随机数
          };
          io.sendFile(_this.$store.state.otherIP,filePath[0],"file",_this.myself.uniqueId,"","",
            "",undefined,_this.user.image,_this.user.nickName,msg.uuid, _this.chatUsers);
          db.insert(msg,function(err,result){
            if(err){
              console.log(err.message);
            }else{
          ...
          if(filePath[0].indexOf("jpg")!= -1){
            ...
            io.sendAudioTemplate(ul,"","",num,filePath[0],msg.isFail,"", "",false,_this.chatUsers);
          }else if(filePath[0].indexOf("mp4")!= -1){
            io.sendVedioTemplate(ul,"","",num,filePath[0],msg.isFail,"", "",false,_this.chatUsers);
          }else{
            io.sendFileTemplate(ul,"","",num,filePath[0],msg.isFail,"", "",false,_this.chatUsers);
          }
          io.scrollBottom("ullMany");
          $('.feature-display').css('display','none');
        });
      ...
      cameraPiker(){
        ...
        GeoBOS.filePicker.pickCamera( function (err, filePath) {
          ...
          if(filePath.indexOf('mp4') != -1){
            ...
            io.sendFile(_this.$store.state.otherIP,filePath,"video",_this.myself.uniqueId,_this.user.uniqueId,_this.myself.image,
              _this.myself.nickName,undefined,_this.user.image,_this.user.nickName,msg.uuid);
            db.insert(msg,function(err,result){
              if(err){
                console.log(err.message);
      ...
      Record_init(){
        ...
        GeoBOS.media.AudioRecorder.getInstance(function (err, recorder) {
          ...
          recorder.on('stop', function (filePath) {
            ...
            io.sendFile(_this.$store.state.otherIP,filePath,"audio",_this.myself.uniqueId,_this.user.uniqueId,
              _this.myself.image,_this.myself.nickName,undefined,_this.user.image,_this.user.nickName,msg.uuid);
            io.sendAudioTemplate(ul,io.formatDateTime(new Date()),_this.myself.image,filePath,msg.isFail,_this.myself.uniqueId,_this.user.uniqueId,false);
            io.scrollBottom("ullMany");
            let db = new DB("Message");
            db.insert(msg,function(err,result){
              if(err){
...
<style>
  .chat-many-list.chat-list li p+div{
    background:#fff;
  ...
  .chat-list .with label:nth-of-type(1){
    font-size: 0.95rem;
    color: #999;
  }
  .chat-list .with label:nth-of-type(2){
    font-size: 1rem;
    color: #999;
  }
  .chat-list .whatsay-text a{background: rgb(230, 230, 10);}
  .chat-list #ullMany .mysay>.whatsay>i,.chat-list #ullMany .mysay>.photoshow>i{
    display: inline-block;
    width: 1.5rem;
    height: 1.5rem;
  ...
  .chat-list #ullMany .mysay>.photoshow>i{
    position: relative;
    left: -1.6rem;
    top: -3em;
  }
