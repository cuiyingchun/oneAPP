	...
	MobileCalendar.prototype = {
        ...
        time_ss.addEventListener('touchend', gearTouchEnd);
          ...
          ss: date.getSeconds()
				...
				if (/^\d{4}-\d{1,2}-\d{1,2}\s\d{2}:\d{2}:\d{2}$/.test(_self.trigger.innerHTML)) {
        ...
        if(_self.idName == "#demo2"){
          if(document.getElementById("count")){
            document.getElementById("count").innerHTML = (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd + " " + (time_hh.length < 2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm+(time_ss.length < 2 ? ":0" : ":") + time_ss;
            tis = document.getElementById("count").innerHTML;
            if (date_mm=0){
              clearInterval(timer)
            }else {
              timer = setInterval(function(){jishi()},1000);
            }
          }
        }else{
          if(document.getElementById("demo3")){
            document.getElementById("demo3").innerHTML = (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd + " " + (time_hh.length < 2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm+(time_ss.length < 2 ? ":0" : ":") + time_ss;
          }
        }
				closeMobileCalendar(e);
			}
        function jishi(){
          var dateNew= new Date(tis).getTime() - new Date().getTime();
          var hours = Math.floor(dateNew / (3600 * 1000));
          var leave2 = dateNew % (3600 * 1000);
          var minutes = Math.floor(leave2 / (60 * 1000));
          var leave3 = leave2 % (60 * 1000);
          var seconds = Math.round(leave3 / 1000);
          if(dateNew>0){
            if(document.getElementById("demo2")){
              document.getElementById("demo2").innerHTML = "T+"+(hours < 10 ? "0" + hours : hours) + ":" + (minutes < 10 ? "0" + minutes : minutes) + ":" + (seconds < 10 ? "0" + seconds : seconds)
            }
            }else{
            if(document.getElementById("demo2")){
              document.getElementById("demo2").innerHTML = "T-"+(Math.abs(hours) < 10 ? "0" + Math.abs(hours) : Math.abs(hours)) + ":" + (Math.abs(minutes) < 10 ? "0" + Math.abs(minutes) : Math.abs(minutes)) + ":" + (Math.abs(seconds) < 10 ? "0" + Math.abs(seconds) : Math.abs(seconds));
